 ...
 */
export function resolveHeadlessShellPath(config) {
...
let pooledCaptureMode = "screenshot";
...
async function probeBeginFrameSupport(browser) {
    ...
    try {
        ...
        const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error("beginFrame probe timeout")), 2000));
 ...
 */
export let _autoBrowserGpuModeCache;
/** Test-only: reset the cached probe result. */
export function _resetAutoBrowserGpuModeCacheForTests() {
 ...
 * render.
 */
export function resolveBrowserGpuMode(mode, options = {}) {
    if (mode !== "auto")
        return Promise.resolve(mode);
    if (_autoBrowserGpuModeCache)
    ...
    _autoBrowserGpuModeCache = (async () => {
        ...
        try {
            ...
            logResolvedBrowserGpuMode("software", `probe failed (${err instanceof Error ? err.message : String(err)})`);
            return "software";
        }
        finally {
...
export async function acquireBrowser(chromeArgs, config) {
    ...
    if (captureMode === "beginframe") {
        ...
        if (!supported) {
            ...
            console.warn("[BrowserManager] HeadlessExperimental.beginFrame unavailable in this Chromium build; falling back to screenshot mode.");
            captureMode = "screenshot";
...
export async function releaseBrowser(browser, config) {
    const enablePool = config?.enableBrowserPool ?? DEFAULT_CONFIG.enableBrowserPool;
    if (!enablePool) {
        await browser.close().catch(() => { });
        return;
...
export function forceReleaseBrowser(browser) {
    if (pooledBrowser && pooledBrowser === browser) {
...
export function buildChromeArgs(options, config) {
    const platform = options.platform ?? process.platform;
    ...
    if (options.captureMode !== "screenshot") {
        chromeArgs.push("--deterministic-mode", "--enable-begin-frame-control", "--disable-new-content-rendering-timeout", "--run-all-compositor-stages-before-draw", "--disable-threaded-animation", "--disable-threaded-scrolling", "--disable-checker-imaging", "--disable-image-animation-resync", "--enable-surface-synchronization");
    }
    if (gpuDisabled) {
