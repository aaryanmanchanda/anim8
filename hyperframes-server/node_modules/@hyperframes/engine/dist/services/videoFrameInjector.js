 ...
 */
function createFrameSourceCache(entryLimit, bytesLimit, frameSrcResolver) {
    const cache = new Map();
 ...
 */
export function createVideoFrameInjector(frameLookup, config) {
    if (!frameLookup)
        return null;
    const entryLimit = Math.max(32, config?.frameDataUriCacheLimit ?? DEFAULT_CONFIG.frameDataUriCacheLimit);
    const bytesLimitMb = Math.max(64, config?.frameDataUriCacheBytesLimitMb ?? DEFAULT_CONFIG.frameDataUriCacheBytesLimitMb);
    ...
    const lastInjectedFrameByVideo = new Map();
    return async (page, time) => {
        ...
        if (activePayloads.size > 0) {
            ...
            for (const [videoId, payload] of activePayloads) {
                ...
                if (lastFrameIndex === payload.frameIndex)
                    continue;
                pendingReads.push(frameCache
        ...
        if (updates.length > 0) {
            await injectVideoFramesBatch(page, updates.map((u) => ({ videoId: u.videoId, dataUri: u.dataUri })));
 ...
 */
export async function hideVideoElements(page, videoIds) {
 ...
 */
export async function showVideoElements(page, videoIds) {
    if (videoIds.length === 0)
        return;
    await page.evaluate((ids) => {
        for (const id of ids) {
 ...
 */
export async function queryVideoElementBounds(page, videoIds) {
    if (videoIds.length === 0)
        return [];
 ...
 */
export async function queryElementStacking(page, nativeHdrIds) {
...
//# sourceMappingURL=videoFrameInjector.js.map