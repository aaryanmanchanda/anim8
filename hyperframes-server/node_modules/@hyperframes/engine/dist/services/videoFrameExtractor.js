...
import { FRAME_FILENAME_PREFIX, ensureCacheEntryDir, lookupCacheEntry, markCacheEntryComplete, readKeyStat, rehydrateCacheEntry, } from "./extractionCache.js";
...
export function parseImageElements(html) {
    const images = [];
    const { document } = parseHTML(unwrapTemplate(html));
...
export async function extractVideoFramesRange(videoPath, videoId, startTime, duration, options, signal, config, 
/**
...
export function codecMayHaveAlpha(codec) {
    return ALPHA_CAPABLE_CODECS.has((codec ?? "").toLowerCase());
}
export function decoderForCodec(codec) {
...
function resolveFrameFormat(metadata, requested) {
    if (requested)
        return requested;
    if (metadata.hasAlpha || codecMayHaveAlpha(metadata.videoCodec))
        return "png";
    return "jpg";
}
/**
...
async function convertVfrToCfr(inputPath, outputPath, targetFps, startTime, duration, signal, config) {
    ...
    const result = await runFfmpeg(args, { signal, timeout });
    if (!result.success) {
        throw new Error(`VFR→CFR conversion failed (exit ${result.exitCode}): ${result.stderr.slice(-300)}`);
    }
}
/**
 ...
 */
export function resolveProjectRelativeSrc(src, baseDir, compiledDir) {
    ...
    if (!fromBaseAbs.startsWith(baseAbs + sep) && fromBaseAbs !== baseAbs) {
        ...
        if (stripped && stripped !== src && !stripped.startsWith("..")) {
            if (compiledDir)
                candidates.push(join(compiledDir, stripped));
...
export async function extractAllVideoFrames(videos, baseDir, options, signal, config, compiledDir) {
        ...
        try {
            ...
            if (!existsSync(videoPath)) {
                ...
                if (!warnedSrcs.has(video.src)) {
                    ...
                    process.stderr.write(`[hyperframes:render] WARNING: video src="${video.src}" ` +
    ...
    const hdrSkippedIndices = new Set();
    ...
    async function tryCachedExtract(video, videoPath, videoDuration, i) {
        ...
        const keyDuration = resolveSegmentDuration(keyInput.end - keyInput.start, keyInput.mediaStart, probedMeta);
        ...
        const result = await extractVideoFramesRange(videoPath, video.id, video.mediaStart, videoDuration, { ...options, format: cacheFormat }, signal, config, lookup.entry.dir);
    ...
    const results = await Promise.all(resolvedVideos.map(async ({ video, videoPath }, i) => {
        if (signal?.aborted) {
            throw new Error("Video frame extraction cancelled");
        }
        try {
            const probedMeta = videoMetadata[i] ?? (await extractMediaMetadata(videoPath));
            const videoDuration = resolveSegmentDuration(video.end - video.start, video.mediaStart, probedMeta);
...
export function getFrameAtTime(extracted, globalTime, videoStart, loop = false, mediaStart = 0) {
    ...
    if (frameIndex < 0 || frameIndex >= extracted.totalFrames)
        return null;
...
export class FrameLookupTable {
    ...
    getFrame(videoId, globalTime) {
        const video = this.videos.get(videoId);
        if (!video)
            return null;
        if (globalTime < video.start || globalTime >= video.end)
            return null;
    ...
    getActiveFramePayloads(globalTime) {
    ...
    cleanup() {
        for (const video of this.videos.values()) {
            ...
            if (video.extracted.ownedByLookup)
                continue;
...
export function createFrameLookupTable(videos, extracted) {
    const table = new FrameLookupTable();
    const extractedMap = new Map();
    for (const ext of extracted)
        extractedMap.set(ext.videoId, ext);
    for (const video of videos) {
        const ext = extractedMap.get(video.id);
        if (ext)
            table.addVideo(ext, video.start, video.end, video.mediaStart, video.loop);
    }
    return table;
}
