...
import { beginFrameCapture, getCdpSession, pageScreenshotCapture, initTransparentBackground, } from "./screenshotService.js";
...
export async function createCaptureSession(serverUrl, outputDir, options, onBeforeCapture = null, config) {
    ...
    if (Number.isFinite(expectedMajor)) {
        ...
        if (Number.isFinite(actualChromiumMajor) && actualChromiumMajor !== expectedMajor) {
            throw new Error(`[FrameCapture] Chromium major mismatch expected=${expectedMajor} actual=${actualChromiumMajor} raw=${browserVersion}`);
        }
    }
    const viewport = {
        width: options.width,
 ...
 */
export function isFontResourceError(type, text, locationUrl) {
    if (type !== "error")
        return false;
    if (!text.startsWith("Failed to load resource"))
        return false;
    return /fonts\.googleapis|fonts\.gstatic|\.(woff2?|ttf|otf)(\b|$)/i.test(`${locationUrl} ${text}`);
}
...
async function applyVideoMetadataHints(page, hints) {
    if (!hints || hints.length === 0)
        return;
    await page.evaluate((metadataHints) => {
        for (const hint of metadataHints) {
            ...
            if (!video.hasAttribute("width"))
                video.setAttribute("width", String(hint.width));
            if (!video.hasAttribute("height"))
                video.setAttribute("height", String(hint.height));
            const computed = window.getComputedStyle(video);
            if (!video.style.aspectRatio &&
...
async function waitForOptionalTailwindReady(page, timeoutMs) {
    const hasTailwindReady = await page.evaluate(`(() => { const ready = window.__tailwindReady; return !!ready && typeof ready.then === "function"; })()`);
    if (!hasTailwindReady)
        return;
...
export async function initializeSession(session) {
    ...
    if (session.captureMode === "screenshot") {
        ...
        const pageReady = await pollPageExpression(page, `!!(window.__hf && typeof window.__hf.seek === "function" && window.__hf.duration > 0)`, pageReadyTimeout);
        if (!pageReady) {
            throw new Error(`[FrameCapture] window.__hf not ready after ${pageReadyTimeout}ms. Page must expose window.__hf = { duration, seek }.`);
        }
        ...
        const videosReady = await pollPageExpression(page, `(() => { const skip = new Set(${skipIdsLiteral}); const vids = Array.from(document.querySelectorAll("video")).filter(v => !skip.has(v.id)); return vids.length === 0 || vids.every(v => v.readyState >= 2); })()`, pageReadyTimeout);
        if (!videosReady) {
            throw new Error(`[FrameCapture] video first frame not decoded after ${pageReadyTimeout}ms. Video elements must reach readyState >= 2 (HAVE_CURRENT_DATA) before capture starts.`);
        }
    ...
    const pageReady = await page.evaluate(`!!(window.__hf && typeof window.__hf.seek === "function" && window.__hf.duration > 0)`);
    if (!pageReady) {
        warmupRunning = false;
        throw new Error(`[FrameCapture] window.__hf not ready after ${pageReadyTimeout}ms. Page must expose window.__hf = { duration, seek }.`);
    }
    ...
    while (Date.now() < videoDeadline) {
        const videosReady = await page.evaluate(`(() => { const skip = new Set(${beginframeSkipIdsLiteral}); const vids = Array.from(document.querySelectorAll("video")).filter(v => !skip.has(v.id)); return vids.length === 0 || vids.every(v => v.readyState >= 2); })()`);
        if (videosReady)
            break;
        await new Promise((r) => setTimeout(r, 100));
    }
 ...
 */
export async function captureFrameToBuffer(session, frameIndex, time) {
...
export function prepareCaptureSessionForReuse(session, outputDir, onBeforeCapture) {
    if (!existsSync(outputDir)) {
        mkdirSync(outputDir, { recursive: true });
    }
