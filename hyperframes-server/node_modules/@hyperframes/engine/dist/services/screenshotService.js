...
export const cdpSessionCache = new WeakMap();
...
const lastFrameCache = new WeakMap();
...
async function sendBeginFrame(client, params) {
    for (let attempt = 0;; attempt++) {
        try {
            ...
            if (isPending) {
                throw new Error(`[BeginFrame] Frame still pending after ${PENDING_FRAME_RETRIES} retries — CPU overloaded by parallel renders. ` +
...
export async function beginFrameCapture(page, options, frameTimeTicks, interval) {
 ...
 */
export async function pageScreenshotCapture(page, options) {
    const client = await getCdpSession(page);
    const isPng = options.format === "png";
    ...
    const clip = dpr > 1 ? { x: 0, y: 0, width: options.width, height: options.height, scale: dpr } : undefined;
...
export async function injectVideoFramesBatch(page, updates) {
    if (updates.length === 0)
        return;
    await page.evaluate(async (items, visualProperties) => {
    ...
    }, updates, [...MEDIA_VISUAL_STYLE_PROPERTIES]);
}
export async function syncVideoFrameVisibility(page, activeVideoIds) {
