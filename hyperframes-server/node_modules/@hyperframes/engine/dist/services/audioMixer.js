...
export function parseAudioElements(html) {
    const elements = [];
...
async function mixAudioTracks(tracks, outputPath, totalDuration, signal, config) {
    const ffmpegProcessTimeout = config?.ffmpegProcessTimeout ?? DEFAULT_CONFIG.ffmpegProcessTimeout;
    ...
    tracks.forEach((track, i) => {
        ...
        filterParts.push(`[${i}:a]atrim=0:${trimDuration},volume=${track.volume},adelay=${delayMs}|${delayMs},apad=whole_dur=${totalDuration}[a${i}]`);
    });
    ...
    await Promise.all(elements.map(async (element) => {
        ...
        try {
            ...
            if (element.type === "video") {
                ...
                const extractResult = await extractAudioFromVideo(srcPath, extractedPath, {
                ...
                const prepResult = await prepareAudioTrack(srcPath, trimmedPath, element.mediaStart, element.end - element.start, signal, config);
