...
import { createCaptureSession, initializeSession, closeCaptureSession, captureFrame, captureFrameToBuffer, getCapturePerfSummary, } from "./frameCapture.js";
...
export function calculateOptimalWorkers(totalFrames, requested, config) {
...
async function executeWorkerTask(task, serverUrl, captureOptions, createBeforeCaptureHook, signal, onFrameCaptured, onFrameBuffer, config) {
    ...
    try {
        ...
        for (let i = task.startFrame; i < task.endFrame; i++) {
            ...
            framesCaptured++;
            if (onFrameCaptured)
                onFrameCaptured(task.workerId, i);
        }
...
export async function executeParallelCapture(serverUrl, workDir, tasks, captureOptions, createBeforeCaptureHook, signal, onProgress, onFrameBuffer, config) {
    const totalFrames = tasks.reduce((sum, t) => sum + (t.endFrame - t.startFrame), 0);
    const workerProgress = new Map();
    for (const task of tasks)
        workerProgress.set(task.workerId, 0);
    ...
    const results = await Promise.all(tasks.map((task) => executeWorkerTask(task, serverUrl, captureOptions, createBeforeCaptureHook, signal, onFrameCaptured, onFrameBuffer, config)));
...
export async function mergeWorkerFrames(workDir, tasks, outputDir) {
