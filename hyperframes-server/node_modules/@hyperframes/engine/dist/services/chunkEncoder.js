...
import { getCachedGpuEncoder, getGpuEncoderName, mapPresetForGpuEncoder, } from "../utils/gpuEncoder.js";
 ...
 */
export function getEncoderPreset(quality, format = "mp4", hdr) {
...
export { detectGpuEncoder } from "../utils/gpuEncoder.js";
export function buildEncoderArgs(options, inputArgs, outputPath, gpuEncoder = null) {
    const { fps, codec = "h264", preset = "medium", quality = 23, bitrate, pixelFormat = "yuv420p", useGpu = false, } = options;
    ...
    if (options.hdr && codec === "h264") {
        console.warn("[chunkEncoder] HDR is not supported with codec=h264 (libx264 has no HDR support). " +
    ...
    if (codec === "h264" || codec === "h265") {
        if (shouldUseGpu) {
            ...
            const colorParams = codec === "h265" && options.hdr
    ...
    if (codec === "h264" || codec === "h265") {
        if (options.hdr) {
            ...
            args.push("-colorspace:v", "bt2020nc", "-color_primaries:v", "bt2020", "-color_trc:v", transferTag, "-color_range", "tv");
        }
        else {
...
export async function encodeFramesFromDir(framesDir, framePattern, outputPath, options, signal, config) {
...
export async function encodeFramesChunkedConcat(framesDir, framePattern, outputPath, options, chunkSizeFrames, signal) {
        ...
        const chunkResult = await new Promise((resolve) => {
            ...
            ffmpeg.on("close", (code) => {
                if (code === 0)
                    ...
                    resolve({ success: false, error: `Chunk ${i} encode failed: ${stderr.slice(-400)}` });
            });
            ffmpeg.on("error", (err) => {
    ...
    const concatResult = await new Promise((resolve) => {
        ...
        ffmpeg.on("close", (code) => {
            if (code === 0)
                ...
                resolve({ success: false, error: `Chunk concat failed: ${stderr.slice(-400)}` });
        });
        ffmpeg.on("error", (err) => {
...
export async function muxVideoWithAudio(videoPath, audioPath, outputPath, signal, config) {
    const outputDir = dirname(outputPath);
    if (!existsSync(outputDir))
        mkdirSync(outputDir, { recursive: true });
...
export async function applyFaststart(inputPath, outputPath, signal, config) {
...
//# sourceMappingURL=chunkEncoder.js.map