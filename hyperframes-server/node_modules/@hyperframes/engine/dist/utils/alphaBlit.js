...
function decodePngRaw(buf, caller) {
    ...
    if (interlace !== 0) {
        throw new Error(`${caller}: Adam7-interlaced PNGs are not supported (interlace method ${interlace})`);
    }
 ...
 */
export function decodePngToRgb48le(buf) {
 ...
 */
export function blitRgba8OverRgb48le(domRgba, canvas, width, height, transfer = "hlg") {
 ...
 */
export function roundedRectAlpha(px, py, w, h, radii) {
    const [tl, tr, br, bl] = radii;
    if (px < tl && py < tl)
        return cornerAlpha(px, py, tl, tl, tl);
    if (px >= w - tr && py < tr)
        return cornerAlpha(px, py, w - tr, tr, tr);
    if (px >= w - br && py >= h - br)
        return cornerAlpha(px, py, w - br, h - br, br);
    if (px < bl && py >= h - bl)
        return cornerAlpha(px, py, bl, h - bl, bl);
    return 1;
}
 ...
 */
export function blitRgb48leRegion(canvas, source, dx, dy, sw, sh, canvasWidth, canvasHeight, opacity, borderRadius) {
    if (sw <= 0 || sh <= 0)
        return;
 ...
 */
export function blitRgb48leAffine(canvas, source, matrix, srcW, srcH, canvasW, canvasH, opacity, borderRadius) {
    const a = matrix[0];
    const b = matrix[1];
    const c = matrix[2];
    const d = matrix[3];
 ...
 */
export function resampleRgb48leObjectFit(source, srcW, srcH, dstW, dstH, fit = "fill", objectPosition) {
    ...
    for (let dy = yMin; dy < yMax; dy++) {
        ...
        for (let dx = xMin; dx < xMax; dx++) {
            ...
            const w01 = ifx * fy;
            const w11 = fx * fy;
            const r = source.readUInt16LE(off00) * w00 +
                source.readUInt16LE(off10) * w10 +
                source.readUInt16LE(off01) * w01 +
                source.readUInt16LE(off11) * w11;
            const g = source.readUInt16LE(off00 + 2) * w00 +
                source.readUInt16LE(off10 + 2) * w10 +
                source.readUInt16LE(off01 + 2) * w01 +
                source.readUInt16LE(off11 + 2) * w11;
            const b = source.readUInt16LE(off00 + 4) * w00 +
                source.readUInt16LE(off10 + 4) * w10 +
                source.readUInt16LE(off01 + 4) * w01 +
                source.readUInt16LE(off11 + 4) * w11;
 ...
 */
export function parseTransformMatrix(css) {
    if (!css || css === "none")
        return null;
    const match2d = css.match(/^matrix\(\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,)]+)\s*\)$/);
    if (match2d) {
...
function warnIfZSignificant(parts) {
    ...
    if (Math.abs(a3) > Z_EPSILON ||
        ...
        console.warn(`[alphaBlit] parseTransformMatrix received a matrix3d with non-trivial 3D components ` +
...
//# sourceMappingURL=alphaBlit.js.map