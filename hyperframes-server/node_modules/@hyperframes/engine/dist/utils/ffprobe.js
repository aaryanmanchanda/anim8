...
function parseProbeJson(stdout) {
    try {
        ...
        throw new Error(`[FFmpeg] Failed to parse ffprobe output: ${e instanceof Error ? e.message : e}`);
    }
}
...
export function extractPngMetadataFromBuffer(buf) {
    if (buf.length < 8 ||
        buf[0] !== 137 ||
        buf[1] !== 80 ||
        buf[2] !== 78 ||
    ...
    while (pos + 12 <= buf.length) {
        ...
        if (chunkType === "cICP" && chunkLen === 4 && !seenIdat) {
            ...
            return {
                ...
                height,
                colorSpace: {
                    colorPrimaries: primariesCode === 9
                        ? "bt2020"
                        : primariesCode === 1
                            ? "bt709"
                            : `unknown-${primariesCode}`,
                    colorTransfer: transferCode === 16
                        ? "smpte2084"
                        : transferCode === 18
                            ? "arib-std-b67"
                            : transferCode === 1
 ...
 */
export async function extractMediaMetadata(filePath) {
    ...
    const probePromise = (async () => {
        ...
        const ffprobeColorSpace = colorTransfer || colorPrimaries || colorSpaceVal
...
export async function extractAudioMetadata(filePath) {
    ...
    const probePromise = (async () => {
        ...
        if (!audioStream)
            throw new Error("[FFmpeg] No audio stream found");
 ...
 */
export async function analyzeKeyframeIntervals(filePath) {
