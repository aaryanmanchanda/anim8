...
export const TIMELINE_REGISTRY_INIT_PATTERN = /window\.__timelines\s*=\s*window\.__timelines\s*\|\|\s*\{\}|window\.__timelines\s*=\s*\{\}|window\.__timelines\s*\?\?=\s*\{\}/i;
export const TIMELINE_REGISTRY_ASSIGN_PATTERN = /window\.__timelines\[[^\]]+\]\s*=/i;
export const WINDOW_TIMELINE_ASSIGN_PATTERN = /window\.__timelines\[\s*["']([^"']+)["']\s*\]\s*=\s*([A-Za-z_$][\w$]*)/i;
export const INVALID_SCRIPT_CLOSE_PATTERN = /<script[^>]*>[\s\S]*?<\s*\/\s*script(?!>)/i;
export function extractOpenTags(source) {
    const tags = [];
    let match;
...
export function extractBlocks(source, pattern) {
    const blocks = [];
    let match;
 ...
 */
export function findHtmlTag(source) {
...
export function findRootTag(source) {
    ...
    const bodyEnd = bodyOpenMatch && bodyCloseMatch && bodyCloseMatch.index > bodyStart
...
export function readAttr(tagSource, attr) {
    if (!tagSource)
        return null;
 ...
 */
export function readJsonAttr(tagSource, attr) {
    if (!tagSource)
        return null;
    const escaped = attr.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const match = tagSource.match(new RegExp(`\\b${escaped}\\s*=\\s*(?:"([^"]*)"|'([^']*)')`, "i"));
...
export function collectCompositionIds(tags) {
    const ids = new Set();
...
export function extractCompositionIdsFromCss(css) {
    const ids = new Set();
    let match;
...
export function getInlineScriptSyntaxError(source) {
    if (!source.trim())
        return null;
    try {
...
export function truncateSnippet(value, maxLength = 220) {
