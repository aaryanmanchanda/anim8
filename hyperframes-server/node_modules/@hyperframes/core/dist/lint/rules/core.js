...
import { readAttr, truncateSnippet, extractCompositionIdsFromCss, getInlineScriptSyntaxError, TIMELINE_REGISTRY_INIT_PATTERN, TIMELINE_REGISTRY_ASSIGN_PATTERN, INVALID_SCRIPT_CLOSE_PATTERN, } from "../utils";
...
function isStudioTimelineElement(tag) {
    ...
    return Boolean(readAttr(tag.raw, "data-start") ||
...
function describeStudioElement(tag) {
    ...
    if (className) {
        ...
        if (primaryClass)
            parts.push(` class="${primaryClass}"`);
    }
    if (compositionId)
        parts.push(` data-composition-id="${compositionId}"`);
    if (dataStart)
        parts.push(` data-start="${dataStart}"`);
    if (dataTrack)
        parts.push(` data-track-index="${dataTrack}"`);
    parts.push(">");
    return parts.join("");
}
export const coreRules = [
        ...
        for (const style of styles) {
            ...
            root.walkRules((rule) => {
                for (const selector of rule.selectors) {
                    if (!selectorTargetsCompositionId(selector, rootCompositionId))
                        continue;
    ...
    ({ tags, rootTag }) => {
        const findings = [];
        for (const tag of tags) {
            if (rootTag && tag.index === rootTag.index)
                continue;
            if (!isStudioTimelineElement(tag))
                continue;
            if (readAttr(tag.raw, "id"))
                continue;
