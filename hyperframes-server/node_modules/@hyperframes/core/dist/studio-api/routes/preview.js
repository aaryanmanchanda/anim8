...
function injectProjectSignature(html, signature) {
    ...
    if (html.includes(`name="${PROJECT_SIGNATURE_META}"`)) {
        return html.replace(new RegExp(`<meta\\s+name=["']${PROJECT_SIGNATURE_META}["'][^>]*>`, "i"), tag);
    }
    if (html.includes("</head>"))
        return html.replace("</head>", `${tag}\n</head>`);
    return `${tag}\n${html}`;
}
export function registerPreviewRoutes(api, adapter) {
    ...
    api.get("/projects/:id/preview", async (c) => {
        ...
        try {
            ...
            if (existsSync(file)) {
                return c.html(injectProjectSignature(readFileSync(file, "utf-8"), resolveProjectSignature(adapter, project.dir)));
            }
            return c.text("not found", 404);
        }
    });
    ...
    api.get("/projects/:id/preview/comp/*", async (c) => {
        ...
        if (!project)
            return c.json({ error: "not found" }, 404);
        const compPath = decodeURIComponent(c.req.path.replace(`/projects/${project.id}/preview/comp/`, "").split("?")[0] ?? "");
    ...
    api.get("/projects/:id/preview/*", async (c) => {
        ...
        if (!project)
            return c.json({ error: "not found" }, 404);
        const subPath = decodeURIComponent(c.req.path.replace(`/projects/${project.id}/preview/`, "").split("?")[0] ?? "");
