...
export function registerWaveformRoutes(api, adapter) {
    api.get("/projects/:id/waveform/*", async (c) => {
        const project = await adapter.resolveProject(c.req.param("id"));
        if (!project)
            return c.json({ error: "not found" }, 404);
        const assetPath = decodeURIComponent(c.req.path.replace(`/projects/${project.id}/waveform/`, "").split("?")[0] ?? "");
        ...
        if (existsSync(cachePath)) {
            try {
                const peaks = JSON.parse(readFileSync(cachePath, "utf-8"));
                return c.json({ peaks });
            }
            catch {
