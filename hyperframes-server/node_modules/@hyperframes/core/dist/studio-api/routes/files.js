...
async function resolveProjectFile(c, adapter, opts) {
...
export function registerFileRoutes(api, adapter) {
    ...
    api.post("/projects/:id/file-mutations/remove-element/*", async (c) => {
        ...
        const filePath = decodeURIComponent(c.req.path.replace(`/projects/${project.id}/file-mutations/remove-element/`, ""));
        if (filePath.includes("\0")) {
    ...
    api.post("/projects/:id/upload", bodyLimit({
        ...
        if (!isSafePath(project.dir, targetDir))
            return c.json({ error: "forbidden" }, 403);
