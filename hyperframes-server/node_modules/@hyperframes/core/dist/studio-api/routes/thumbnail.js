...
export function registerThumbnailRoutes(api, adapter) {
    api.get("/projects/:id/thumbnail/*", async (c) => {
        ...
        let compPath = decodeURIComponent(c.req.path.replace(`/projects/${project.id}/thumbnail/`, "").split("?")[0] ?? "");
        if (compPath && !compPath.includes("."))
            compPath += ".html";
        ...
        if (!vpWidth) {
            ...
            if (existsSync(htmlFile)) {
                ...
                if (wMatch?.[1])
                    compW = parseInt(wMatch[1]);
                if (hMatch?.[1])
                    compH = parseInt(hMatch[1]);
            }
        }
        const previewUrl = compPath === "index.html"
