...
function collectProjectSignatureFiles(projectDir, dir, files) {
    ...
    for (const entry of entries) {
        if (SIGNATURE_EXCLUDED_DIRS.has(entry))
            continue;
        const file = resolve(dir, entry);
        if (!isPathWithin(projectDir, file))
            continue;
        let stat;
        try {
            stat = lstatSync(file);
...
export function createProjectSignature(projectDir) {
...
//# sourceMappingURL=projectSignature.js.map